...
const { calculateMiningBonus } = require('../game/gameLogic');

const { getCardsInMiningArea,moveToBackpack  } = require('../db/database'); // 使用不同的名称
...
async function createBagForUser(userId) {
    return new Promise((resolve, reject) => {
        const query = 'INSERT INTO user_bag (user_id, card_type, quantity) VALUES (?, ?, ?)';
        db.query(query, [userId, 'noCard', 0], (error, results) => {
            if (error) {
                reject(error);
            } else {
                resolve(results);
...
// exports.moveToMining = async (req, res) => {
//     const userId = req.session.userId;
//     const cardType = req.body.cardType;
//     const quantity = req.body.quantity;

//     // 获取用户在挖矿区域的所有卡片
//     const cardsInMiningArea = await getCardsInMiningArea(userId);

//     // 计算挖矿奖励
//     const bonus = calculateMiningBonus(cardsInMiningArea);

//     // 使用bonus进行其他操作，例如更新用户的挖矿奖励

//     await db.moveToMiningArea(userId, cardType, quantity);
//     res.redirect('/play');
// };

// exports.moveToBackpack = async (req, res) => {
//     const userId = req.session.userId;
//     const cardType = req.body.cardType;
//     const quantity = req.body.quantity;

//     // 获取用户在挖矿区域的所有卡片
//     const cardsInMiningArea = await getCardsInMiningArea(userId); // 使用新名称

//     // 计算移回背包前的挖矿奖励
//     const previousBonus = calculateMiningBonus(cardsInMiningArea);

//     // 执行移回背包操作
//     await moveToBackpack(userId, cardType, quantity);

//     // 重新获取用户在挖矿区域的所有卡片
//     const updatedCardsInMiningArea = await getCardsInMiningArea(userId); // 使用新名称

//     // 计算移回背包后的挖矿奖励
//     const updatedBonus = calculateMiningBonus(updatedCardsInMiningArea);

//     // 使用previousBonus和updatedBonus进行其他操作，例如更新用户的挖矿奖励

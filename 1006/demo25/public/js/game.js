...
const cardDescriptions = {
    ...
    AE: '<img src="/images/pok/AE.png" alt="Card AE">',
    AF: '<img src="/images/pok/AF.png" alt="Card AF">',
    AG: '<img src="/images/pok/AG.png" alt="Card AG">',
    AH: '<img src="/images/pok/AH.png" alt="Card AH">',
    AI: '<img src="/images/pok/AI.png" alt="Card AI">',
    AJ: '<img src="/images/pok/AJ.png" alt="Card AJ">',
    AK: '<img src="/images/pok/AK.png" alt="Card AK">',
    AL: '<img src="/images/pok/AL.png" alt="Card AL">',
    AM: '<img src="/images/pok/AM.png" alt="Card AM">',
    AN: '<img src="/images/pok/AN.png" alt="Card AN">',
    AO: '<img src="/images/pok/AO.png" alt="Card AO">',
...
function drawCard02(quantity, costPerCard) {
    const totalCost = quantity * costPerCard;   
    console.log('coins2', coins);

    if (coins >= totalCost) {
        purchaseCards2(quantity, costPerCard);
    } else {
        alert("金幣不足，無法抽卡。");
    }
}
...
function purchaseCards2(quantity, costPerCard) {
    fetch('/shop2', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
...
function getCoinsAndCardsFromServer() {
    ...
    fetch('/getUserData')
        ...
        .then(data => {
            ...
            updateCardACount(data.cardACount);

            localStorage.setItem('coins', data.coins);
            localStorage.setItem('totalCards', data.totalCards);
            console.log('data.totalCards1111111111111',data.totalCards);
            console.log('data.cardCount22222222222',data.cardACount);
        })
        .catch(error => {
...
// 假設您的A卡數量顯示在一個ID為'cardACount'的元素中
function updateCardACount(cardACount) {
    const cardACountElement = document.getElementById('cardACount');
    if (cardACountElement) {
        cardACountElement.textContent = cardACount;
    }
}
function updatepn(totalCards) {
    console.log('totalCards111111',totalCards)
    console.log('updatepn 函數被調用了'); 
    const pnElement = document.getElementById('pn');
    if (pnElement) {
        pnElement.textContent = totalCards;
    }
}

...
app.post('/updateTotalResources', (req, res) => {
    const userId = req.session.userId; 
    const newTotalResources = req.body.totalResources;

    db.query('UPDATE users SET totalResources = ? WHERE user_id= ?', [newTotalResources, userId], (error, results) => {
        if (error) {
...
app.get('/getTotalResources', (req, res) => {
    const userId = req.session.userId;

    db.query('SELECT totalResources FROM users WHERE user_id = ?', [userId], (error, rows) => {
        if (error) {
        ...
        if (rows.length > 0) {
            res.send({ totalResources: rows[0].totalResources });
            console.log('totalResources456456456',rows[0].totalResources)
        } else {
...
app.post('/moveSelectedToMining', async (req, res) => {
    ...
    db.beginTransaction(async (err) => {
        ...
        db.query('SELECT card_id, card_type FROM user_bag WHERE user_id = ? AND card_id IN (?)', [userId, selectedCardId], (error, rows) => {
            if (error) {
                return db.rollback(() => {
                    res.send({ success: false, message: error.message });
                });
                ...
                const insertQueries = selectedCards.map(card => {
                    return new Promise((resolve, reject) => {
                        const query = `
                            ...
                            ON DUPLICATE KEY UPDATE quantity = quantity + VALUES(quantity)
                        `;
                        db.query(query, [card.card_id, userId, card.card_type, 1], (error) => {  // Assuming quantity is 1
                            if (error) reject(error);
                            else resolve();
                        });
                    });
...
app.post('/moveSelectedToBackpack', async (req, res) => {
    ...
    db.beginTransaction(async (err) => {
        ...
        db.query('SELECT card_id, card_type FROM mining_area WHERE user_id = ? AND card_id IN (?)', [userId, selectedCardId], (error, rows) => {
            if (error) {
                return db.rollback(() => {
                    res.send({ success: false, message: error.message });
                });
